...
namespace Valve.VR.InteractionSystem
{
    ...
    public class Hand : MonoBehaviour
    {
        ...
        [Header("Limites size")]
        public float limitedSizeMax = 2.5f;
        public float limitedSizeMin = 0.5f;
        ...
        protected virtual void TwoHandGrabbingUpdate(SteamVR_Behaviour_Pose updatedPose,
            SteamVR_Input_Sources updatedSource)
        {
            ...
            if (attachedObject != null && twoHandGrab)
            {
                ...
                if (newScale.x < limitedSizeMin)
                    newScale = new Vector3(limitedSizeMin, limitedSizeMin, limitedSizeMin);
                if (newScale.x > limitedSizeMax)
                    newScale = new Vector3(limitedSizeMax, limitedSizeMax, limitedSizeMax);

                if (newScale.x >= 0.5f && newScale.x <= 2f)  // Ограничение скейла
                {
        ...
        public void DetachObject(GameObject objectToDetach, bool restoreOriginalParent = true)
        {
            ...
            if (index != -1)
            {
                ...
                if (otherHand.twoHandGrab)
                {
                    ...
                    otherHand.AttachObject(otherHand.currentAttachedObject,
                        otherHand.GetBestGrabbingType(GrabTypes.None), otherHand.initialAttachmentFlags);
                }
